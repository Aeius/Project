...
//	가상주소  http://localhost:8080/myweb2/pages/faqList	
	...
	public String chart(Model model, HttpSession session) {
		...
		if (result.equals("admin")) { // 2번째
			...
			// update product set product_sellcount=500, product_likecount=670 where
			// product_idx=9;
...
//	      System.out.println(nb.getNotice_content());
	...
	// ------------------ 리뷰 내용 보기 ---------------------
	@RequestMapping(value = "/reviewContent.ad", method = RequestMethod.GET)
	public String reviewContent(Model model, HttpServletRequest request, HttpSession session) {
...
//		if (result.equals("admin")) { // 2번째
			...
			ReviewBean review  = adminService.getReview(review_idx);
			model.addAttribute("review", review);
			return "/AdminLTE-master/pages/reviewContent";
...
//		System.out.println(coupon_idx);
	...
	// ------------------ 쿠폰 자동 관리버튼 ---------------------
	@RequestMapping(value = "/couponAutoManage.ad", method = RequestMethod.GET)
	public String couponAutoManage(HttpServletRequest request) {
		adminService.couponActivateInOnce(); // 모든 쿠폰 활성
		adminService.couponExpireInOnce(); // couponActivateInOnce()메서드로 인해 활성화 된
											// 모든 쿠폰 중 기간이 만료 된 쿠폰을 비활성화
		return "redirect:/couponList.ad";
	}
